...
```{r}
table(sanjose$use_of_force_reason)
table(sanjose$use_of_force_reason,sanjose$use_of_force_description)
```
...
### Variation #2
```{r}
round(prop.table(table(sanjose$subject_race, sanjose$arrest_made),1),2)
```

or Variation #2a
```{r}
prop.table(table(sanjose$search_conducted, sanjose$subject_race),2)
```

Q1 for first exam - sample of 1000 traffic stops in San Jose and breakdown by search and subject's race.
...
sanjose %>% 
  ...
  filter(subject_race %in% c("asian/pacific islander","black","hispanic", "white")) %>%
  filter(search_conducted %in% c("FALSE", "TRUE")) %>% 
  droplevels() %>% 
  ...
  kable() %>% 
    kable_styling(bootstrap_options = "striped",
                  full_width = T, 
                  position="left") %>% 
    add_header_above(c("","Search was conducted"=2))
...
ggplot(data=sanjose_ped, aes(x=subject_race, fill=contraband_found))+
  geom_bar(position="dodge")
```
...
```{r}
prop.table(table(sanjose$year, sanjose$type),1)
```
...
## Advanced Treatment facilitated by `purrr` package
```{r}
library(purrr)
sanjose %>% filter(type=="vehicular") %>% select(subject_race, search_conducted, arrest_made, outcome) %>%  split(.$subject_race) %>% map(summary)
...
```{r}
sjpd <- sanjose %>% 
  filter(year == "2017", type == "vehicular") %>% 
  select( subject_race, search_conducted, contraband_found, citation_issued, arrest_made, reason_for_stop, use_of_force_description)
  


sjpd$search_conducted <- as.factor(sjpd$search_conducted)
sjpd$contraband_found <- as.factor(sjpd$contraband_found)
sjpd$arrest_made <- as.factor(sjpd$arrest_made)
sjpd$citation_issued <- as.factor(sjpd$citation_issued)
glimpse(sjpd)
```

```{r}
library(jsonlite)
write( toJSON(sjpd, dataframe = 'columns'), 'sjpd_2017.json')
write.csv(sjpd, file="sjpd_2017.csv")
```
